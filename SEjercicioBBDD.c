...
int main(int argc, char **argv)
{
	...
	if (row == NULL)
		...
		// dado por el usuario
		printf ("\nEscribe el nombre del jugador para ver su puntuaciÛn m·xima:\n"); 
		scanf ("%s", nombre);
		// construimos la consulta SQL
		sprintf (consulta,"SELECT MAX(GAME.DURATION) FROM (GAME,PLAYER,BRIDGE) WHERE PLAYER.USERNAME = '%s' AND GAME.ID = BRIDGE.ID_GA AND (PLAYER.ID = BRIDGE.ID_PLY1 OR PLAYER.ID = BRIDGE.ID_PLY2)",nombre); 
		// hacemos la consulta 
